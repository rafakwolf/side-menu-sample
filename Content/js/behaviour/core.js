...
hbsis.wms.CrudForm = Class.extend({
    ...
    editMarkup:
        "<a class=\"label label-default edit\" href=\"#edit\" data-target=\"#save-modal\" data-toggle=\"modal\" title=\"Editar\">" +
            "<i class=\"fa fa-pencil\"></i></a> ",
    editPriority:
        "<a class=\"label label-default edit\" href=\"#edit\" data-target=\"#save-modal\" data-toggle=\"modal\" title=\"Priorizar\">" +
            "<i class=\"fa fa-arrows-v \"></i></a> ",
    deleteMarkup: "<a class=\"label label-danger delete\" href=\"#delete\" title=\"Remover\">" +
    ...
    init: function(opts) {
        ...
        //this.clearOnSave = ko.observable(true);
        this.dirty = ko.observable(false);
        this.document = $(document);

        var self = this;

        /* Trata o "Enter" no formul�rio */
        ...
        /* Marca o dataTable para for�ar um refresh no pipeline 
         * quando alguma opera��o de create/update/delete � feita
         */
        this.dirty.subscribe(function(newValue) {
            if (newValue === true && self.datatable) {
                self.datatable.fnClearPipeline();
            }
        });

        /* Ao fechar o modal do formul�rio limpa o formul�rio,
         * sai do modo de edi��o e, se foi feita alguma altera��o,
        ...
        this.modal.on("hidden.bs.modal",
            function(e) {
                self.clearSaveForm();
                self.editMode(false);
                var existeTableLoader = (self.settings.dataTableConfig.hasOwnProperty("loader")) 
                    && (self.settings.dataTableConfig.loader !== null) 
                    && (self.settings.dataTableConfig.loader !== undefined);
                if(existeTableLoader)
                    self.settings.dataTableConfig.loader.fadeIn("fast"); 
                ...
                if(existeTableLoader)
                    self.settings.dataTableConfig.loader.fadeOut("fast");
            });
    ...
    /* Retorna a configura��o da datatable
     * @virtual
     * @return Object
     */
    ...
    initDatatable: function() {
        ...
        var datatable = $(self.settings.datatableId);
        datatable = datatable.on('init.dt',
            function() {
                $('.dataTables_filter input').addClass('form-control').attr('placeholder', resources.get('Search...'));
                $('.dataTables_length select').addClass('form-control');
            });

        datatable = datatable.dataTable($.extend({
                'bProcessing': true,
                'bServerSide': true,
                'fnServerData': $.fn.dataTable.pipeline({ url: self.settings.datatableUrl }),
                "fnPreDrawCallback": $.fn.dataTable.minLengthFilter({ minLength: 3 }),
                "fnCreatedRow": function(nRow, aData, iDataIndex) { $(nRow).attr('id', aData.Id); },
                "bDeferRender": true,
                /*"aoColumns": self.settings.dataTableConfig.aoColumns,
                "aoColumnDefs": self.settings.dataTableConfig.aoColumnDefs,*/
                "oLanguage": { "sUrl": self.settings.datatableLang },
                "iDisplayStart": self.getCurrentPage(),
                "iDisplayLength": self.getCurrentLength()
            },
            self.settings.dataTableConfig));
    ...
    applyDatabind: function(ko) {
        ...
        if (self.datatable != null) {
            self.document.on("click",
                self.settings.datatableId + " " + self.settings.deleteSelector,
                function(e) {
            ...
            self.document.on("click",
                self.settings.datatableId + " " + self.settings.editSelector,
                function(e) {
                    var row = $(this).parents('tr')[0];
                    var model = self.datatable.fnGetData(row);
                    self.edit(model);
    ...
    obterUrlSalvar: function (id) {
        return this.form.attr('action') + (this.editMode() ? "/" + id : "");
    },
    save: function (e) {
        this.clearErrorAlert();
        var valid = true;
...
hbsis.wms.Helpers = {
    ...
    initSingleDatetimePicker: function (opts) {
        ...
        $(settings.field).daterangepicker(settings.config, function (start) {

            $(settings.field + ' span').html(start.format('MMMM D, YYYY'));

            if ($.isFunction(settings.callback))
                settings.callback(start);
        });
    },
...
$(function () {
    ...
    // Seleciona a op��o ativa no menu
...
$(function () {
    ...
    //flag que indica quando um binding n�o deve ser aplicado
