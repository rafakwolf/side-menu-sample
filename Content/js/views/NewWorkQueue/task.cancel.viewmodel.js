...
hbsis.wms.settings.WorkQueueViewModel = hbsis.wms.CrudForm.extend({
    startDate: moment(),
    endDate: moment(),
    //Delete/WorkType/{workType:int}/{id:guid}
    deleteUrl: function(model) {
        return "Delete/WorkType/" + model.WorkTypeValue + "/Id/" + model.Id;
    },
                ...
                },
                {
                    ...
                    "bSearchable": false, "mRender": function (data, type, row) {
                        if (row.Status == 'Assigned') {
                            return resources.get(row.Status) + " <a class=\"label label-primary unassoc\" rel=\"" + row.Id + "\"><i class=\"fa fa-times\"></i></a>";
